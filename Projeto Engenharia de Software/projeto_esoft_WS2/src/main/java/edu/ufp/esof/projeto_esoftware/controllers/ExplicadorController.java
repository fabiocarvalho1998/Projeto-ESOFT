...
public class ExplicadorController {
    ...
    public ResponseEntity<List<Explicador>> getExplicadores(@RequestParam Map<String,String> query){
        Long idUniversidade=null;
        try {
            idUniversidade= Long.parseLong(query.get("universidade") );
        }catch (NumberFormatException nfe){
            nfe.printStackTrace();

        }
//        Long idUniversidade = Long.parseLong(query.get("universidade") != null ? query.get("universidade") : "-1");
        ...
        if(idUniversidade!=null) {
            ...
            if (optionalUniversidade.isPresent()) {
                ...
                ResponseEntity<List<Explicador>> responseEntity = makeRequest(path, HttpMethod.GET, nullBody, List.class);
                this.logger.info("Pedido GET Enviado!");
        ...
        List<Explicador> explicadors=new ArrayList<>();
        for(Universidade uni:universidadeService.getAllUniversidades()){
            String path = uni.getIp().concat("explicador?cadeira=" + idCadeira + "&dia=" + dia + "&inicio=" + inicio + "&fim=" + fim);
            ...
            try {
                ...
                if(responseEntity.getBody()!=null) {
                    explicadors.addAll(responseEntity.getBody());
                }
            }catch(Exception ce){
                ce.printStackTrace();
            }
        ...
        return ResponseEntity.ok(explicadors);
